...
def has_sublist(s, sublist):
    ...
    if s == empty and sublist != empty:
        return False
    elif s == empty or sublist == empty:
        return True
    elif first(sublist) != first(s):
...
def has_kitty_gene(dna):
    ...
    """
    "*** YOUR CODE HERE ***"
    gene = link('C', link('A', link('T')))
    gene = link('C', link('A', link('T', gene)))
...
def count_change(amount, denominations):
    ...
    if amount == 0:
        return 1
    add_first = count_change(amount-first(denominations), denominations)
    add_rest = count_change(amount, rest(denominations))
    return add_first + add_rest
